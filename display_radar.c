...
int server() {
   ...
   if ((attach = name_attach(NULL, dis_point, 0)) == NULL) {
       return EXIT_FAILURE;
   }
...
//	   /* Here are the messages that you will work on, i.e., messages that will have a meaning
	   if (msg.hdr.type == 0x00) {
	      if (msg.hdr.subtype == 0x01) {
              ...
              printf("Plane within Radar");
              printf(" %d %d %d %d %d %d %d\n ", msg.sendarr[0].id, msg.sendarr[0].x, msg.sendarr[0].z, msg.sendarr[0].y,msg.sendarr[0].x_speed, msg.sendarr[0].y_speed,msg.sendarr[0].z_speed);
